...
$(document).on("click", "#event-window-research-ok", function (e) {
  $.post(
    "index.php?method=research&json=1",
    { rid: $("#event-window-select-research").val() },
    function (data) {
      resp = $.parseJSON(data);
      if (resp.status == "ok") {
...
$(document).on(
  "click",
  "#event-window-research-cancel, #event-window-research .btn-close",
  function (e) {
    $("#event-window-research").hide();
    get_next_event($("#event-window-research").attr("eid"));
  },
...
$(document).on(
  "click",
  "#event-window-build-tocity, #event-window-city .btn-close",
  function (e) {
    $.post(
      "index.php?method=event&json=1",
      { del: $("#event-window-city").attr("eid") },
      function (data) {
        $("#event-window-city").hide();
        $("#modal-backdrop").hide();
        resp = $.parseJSON(data);
...
$(document).on("keydown", "body", function (e) {
  ...
  if (
    ...
    e.originalEvent.code == "NumpadEnter"
  ) {
    next_step();
  }
  return true;
...
$(document).on("click", "body", function (e) {
  if (
    $(e.target).closest("#cellmenu").length == 0 &&
...
$(document).on("click", "#open-research", function (e) {
  $.post("index.php?method=research", {}, function (data) {
    ...
    $("#research-window").toggle();
    $("#research-window").css("background-image", "");
