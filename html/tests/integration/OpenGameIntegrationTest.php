...
class OpenGameIntegrationTest extends TestBase
{
    ...
    public function testFullGameOpeningProcess(): void
    {
        ...
        $this->assertEquals(
            $game["id"],
            $_SESSION["game_id"],
            "ID игры должен быть установлен в сессии",
        );
        $this->assertEquals(
            $players[1]["id"],
            $_SESSION["user_id"],
            "ID пользователя должен быть установлен в сессии",
        );
    ...
    public function testGameOpeningSessionPersistence(): void
    {
        ...
        $this->assertNotNull(
            $sessionGame,
            "Должна быть получена игра из сессии",
        );
        $this->assertNotNull(
            $sessionUser,
            "Должен быть получен пользователь из сессии",
        );
    ...
    public function testGameOpeningWithDifferentConfigurations(): void
    {
        ...
        foreach ($configurations as $config) {
            ...
            $this->assertEquals(
                $game["id"],
                $_SESSION["game_id"],
                "Сессия должна быть установлена для конфигурации: {$config["name"]}",
            );
            $this->assertEquals(
                $user["id"],
                $_SESSION["user_id"],
                "Пользователь должен быть установлен для конфигурации: {$config["name"]}",
            );
        }
    }
    ...
    public function testPerformanceGameOpening(): void
    {
        ...
        $this->assertLessThan(
            1.0,
            $executionTime,
            "Открытие игры должно занимать менее 1 секунды",
        );
        $this->assertEquals($game["id"], $_SESSION["game_id"]);
    ...
    public function testGameOpeningObjectRelationships(): void
    {
        ...
        $this->assertCount(
            4,
            $sessionGame->users,
            "Игра должна содержать 4 пользователей",
        );
        ...
        $this->assertArrayHasKey(
            $sessionUser->id,
            $sessionGame->users,
            "Текущий пользователь должен быть в списке пользователей игры",
        );
        ...
        $this->assertEquals(
            $sessionGame->id,
            $sessionUser->game,
            "Пользователь должен принадлежать открытой игре",
        );
    }
