...
class Unit {
	...
	public function save() {
		$data = ['user_id' => $this->user->id,
				 'type' => $this->type->id];
		foreach (['x', 'y', 'planet', 'health', 'health_max', 'points', 'mission_points', 'auto'] as $field) {
	...
	public function __construct($data) {
		...
		foreach (['x', 'y', 'planet', 'health', 'health_max', 'mission_x', 'mission_y', 'points', 'mission_points', 'auto'] as $field) {
			if (isset($data[$field])) {
	...
	public function get_mission_types($x = null, $y = null) {
		if (is_null($x)) $x = $this->x;
		if (is_null($y)) $y = $this->y;
		$result = [];
	...
	public function move_to($cell)
    {
        ...
        if (($cell->road || $cell->city) && ($cell_from->road || $cell_from->city)) {
    ...
    public function add_order($mission, $target_x = 'NULL', $target_y = 'NULL', $number = false) {
	    ...
	    MyDB::insert('mission_order',
            ['unit_id' => $this->id, 'number' => $number, 'type' => $mission, 'target_x' => $target_x, 'target_y' => $target_y]);
	    return $number;
    }
	    ...
	    if ($this->mission) {
            ...
            if (!$order && $this->auto == 'work') {
                ...
                if (MyDB::query("SELECT count(unit_id) FROM mission_order WHERE unit_id = :uid",
                    ...
                    $paths = [];
                    foreach ($cities as $city) { //Проверяем расстояния до своих городов
                        $path = $this->calculate_path(Cell::get($this->x, $this->y), Cell::get($city->x, $city->y));
                        if ($path) {
                            $paths[$city->id] = $path;
                        }
                    }
                    ...
                    while (count($paths) > 0 && !$auto_ok) {//Пробуем все города от ближайшего
                        $path = array_shift($paths);
                        $paths_to = [];
                        foreach ($cities as $city_to) {
                            if ($city == $city_to) continue;
                            $path_to = $this->calculate_path(Cell::get($city->x, $city->y), Cell::get($city_to->x, $city_to->y));
                            if ($path_to) {
                            ...
                            while (count($paths_to) > 0) { // Перебираем дороги ко всем городам от него
                                $path_to = array_shift($paths_to);
                                foreach ($path_to['path'] as $cell) {
                                    if (!$cell->city && !$cell->road && ($cell->owner == $this->user || !$cell->owner)) {
                                        $need_road = true; //Строим тут дорогу
                                        $move_path = $this->calculate_path(Cell::get($ux, $uy), $cell);
                                        array_shift($move_path);
                                        foreach ($move_path['path'] as $move) {
                                            $number++;
                                            $this->add_order('move', $move->x, $move->y);
                                        }
                                        $number++;
                                        $this->add_order('build_road', 'NULL', 'NULL', $number);
                                        $ux = $cell->x;
                                        $uy = $cell->y;
                                    }
                                }
                    ...
                    if (!$auto_ok) { //Не нашли где строить дороги между городами
                    ...
                    if (!$auto_ok) { //Не нашли где строить дороги около городов
                        $max_path = GameConfig::$WORK_DIST1 + 1;
                        $path = false;
                        $mine = MissionType::get('mine');
                        $irrigation = MissionType::get('irrigation');
                        $cell_types = array_merge($mine->cell_types, $irrigation->cell_types);
                        ...
                        if ($path) {
                            ...
                            if (in_array($irrigation->cell_types, $cell->type->id)) {
                                $this->add_order('irrigation', 'NULL', 'NULL', $number);
                            }
                            if (in_array($mine->cell_types, $cell->type->id)) {
                                $this->add_order('mine', 'NULL', 'NULL', $number);
                            }
                        }
                    }
                }
    ...
    public function calculate_path($cell1, $cell2, $max_path = 200) {
        ...
        while (count($cells_next) > 0) {
            ...
            if (isset($cells_path[$next['cell']->x.'_'.$next['cell']->y]) &&
            ...
            $cells_around = Cell::get_cells_around($next['cell']->x, $next['cell']->y, 3, 3);
            foreach ($cells_around as $row) foreach ($row as $map_near) {
                if ($map_near == $next['cell']) continue;;
                if (!isset($this->type->can_move[$map_near->type->id]) && !$map_near->city) continue; //Сюда пути нет
                if (($next['cell']->road || $next['cell']->city) && ($map_near->road || $map_near->city)) {
                    $dist_near = GameConfig::$ROAD_MOVE_POINTS; //Есть дорога
                } else {
                    if ($map_near->city) {
                        $dist_near = $this->type->can_move['city'];
                    } else {
                        $dist_near = $this->type->can_move[$map_near->type->id];
                    }
                }
                $cells_next[] = ['cell' => $map_near, 'prev' => $next['cell'], 'dist' => $next['dist'] + $dist_near];
            }
        }
        if ($dist <= $max_path) { //Нашли путь
            $x = $cell2->x;
            $y = $cell2->y;
            $path = [$cell2];
            $i = 0;
