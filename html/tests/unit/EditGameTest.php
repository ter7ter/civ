...
class EditGameTest extends TestBase
{
    ...
    public function testEditBasicGameParameters(): void
    {
        ...
        $this->createTestUser(['login' => 'Игрок1', 'game' => $game['id'], 'turn_order' => 1]);
        $this->createTestUser(['login' => 'Игрок2', 'game' => $game['id'], 'turn_order' => 2]);
        ...
        $this->assertFalse(
            $error,
            "Не должно быть ошибок при редактировании базовых параметров: " . (is_string($error) ? $error : '')
        );
    ...
    public function testEditGameNameOnly(): void
    {
        ...
        $this->assertFalse(
            $error,
            "Не должно быть ошибок при редактировании только названия: " . (is_string($error) ? $error : '')
        );
    ...
    public function testEditMapSize(): void
    {
        ...
        $this->assertFalse(
            $error,
            "Не должно быть ошибок при редактировании размеров карты: " . (is_string($error) ? $error : '')
        );
    ...
    public function testLoadGameDataForEditing(): void
    {
        ...
        $this->createTestUser(['login' => 'Первый игрок', 'game' => $game['id'], 'turn_order' => 1]);
        $this->createTestUser(['login' => 'Второй игрок', 'game' => $game['id'], 'turn_order' => 2]);
        $this->createTestUser(['login' => 'Третий игрок', 'game' => $game['id'], 'turn_order' => 3]);
        ...
        $this->assertEquals(['Первый игрок', 'Второй игрок', 'Третий игрок'], $data['users']);
    }
    ...
    public function testDataPreservationOnValidationError(): void
    {
        ...
        $this->assertEquals(150, $data["map_w"], "Ширина карты должна сохраниться");
        $this->assertEquals(200, $data["map_h"], "Высота карты должна сохраниться");
        $this->assertEquals("concurrently", $data["turn_type"], "Тип ходов должен сохраниться");
        $this->assertEquals(['Игрок1', 'Игрок2'], $data["users"], "Список игроков должен сохраниться");
    }
    ...
    public function testXSSInEditedGameName(): void
    {
        ...
        $this->assertFalse(
            $error,
            "Не должно быть ошибок при редактировании игры с XSS в названии: " . (is_string($error) ? $error : '')
        );

        // Проверяем, что название было экранировано в БД
        $updatedGame = $this->getLastRecord("game");
        $this->assertEquals(htmlspecialchars($maliciousName), $updatedGame['name'], "Название игры в БД должно быть экранировано.");
    }
    ...
    public function testVeryLongStringsInEdit(): void
    {
        ...
        $updatedGame = $this->getLastRecord("game");
        $this->assertEquals($longName, $updatedGame['name'], "Длинное имя игры должно быть сохранено полностью.");
    }
