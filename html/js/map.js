  ...
  show_menu: function (point_x, point_y) {
    ...
    for (var unit in this.units) {
      $("#cellmenu ul").append(
        '<li unit="' + unit + '">' + this.units[unit].title + "</li>",
      );
    }
...
var map = {
  ...
  draw: function () {
    ...
    for (var y = 0; y < map.height; y++) {
      ...
      for (var x = 0; x < map.width; x++) {
        ...
        if (map.cells[x][y] == undefined || map.cells[x][y].title == "none") {
          el = $(
            '<div class="map_cell map_cell_none"' + cdata + ">&nbsp</div>",
          );
        } else {
          ...
          if (cell.owner_name) {
            owner =
              '<div class="map-cell-owner-color" style="background-color:' +
              cell.owner_color +
              '"></div>';
          }
          var road = "";
          ...
          if (cell.resource_id) {
            resource =
              '<div class="map-cell-resource" style="background-image: url(img/res/' +
              cell.resource_id +
              '.png);"></div>';
          }
          var improvement = "";
          if (cell.improvement) {
            resource =
              '<div class="map-cell-improvement" style="background-image: url(img/improvement/' +
              cell.improvement +
              '.png);"></div>';
          }
          if (map.cells[x][y].city) {
            ...
            el +=
              '<div class="map-city-population">' +
              cell.city.population +
              "</div>";
            el +=
              '<div class="map-city-title">' +
              cell.city.title.substr(0, 7) +
              "</div>";
            el += "</div>";
            el = $(el);
          } else {
            el = $(
              '<div class="map_cell map_cell_' +
                cell.type +
                '"' +
                cdata +
                ">" +
    ...
    for (var y = 0; y < map.height; y++) {
      for (var x = 0; x < map.width; x++) {
        ...
        if (cell.owner_name) {
          ...
          if (
            y < map.height - 1 &&
            cell.owner_name != map.cells[x][y + 1].owner_name
          ) {
          ...
          if (
            x < map.width - 1 &&
            cell.owner_name != map.cells[x + 1][y].owner_name
          ) {
        ...
        if (cell.road) {
          ...
          if (
            x > 0 &&
            y < map.height - 1 &&
            (map.cells[x - 1][y + 1].road || map.cells[x - 1][y + 1].city) &&
            !map.cells[x - 1][y].road &&
            !map.cells[x - 1][y].city &&
            !map.cells[x][y + 1].road &&
            !map.cells[x][y + 1].city
          ) {
            class_name = class_name + "1";
          }
          if (
            y < map.height - 1 &&
            (map.cells[x][y + 1].road || map.cells[x][y + 1].city)
          ) {
          ...
          if (
            x < map.width - 1 &&
            (map.cells[x + 1][y].road || map.cells[x + 1][y].city)
          ) {
    ...
    $(
      '.map_cell[coordx="' + map.select_x + '"][coordy="' + map.select_y + '"]',
  ...
  show_cell_info: function () {
    ...
    if (
      selected_unit &&
      selected_unit.x == this.select_x &&
      selected_unit.y == this.select_y
    ) {
      unit_id = selected_unit.id;
    }
    $.post(
      "index.php?method=cellinfo",
      { x: map.select_x, y: map.select_y, unit_id: unit_id },
      function (data) {
  ...
  draw_path: function (path) {
    for (var n = 0; n < path.length; n++) {
      ...
      map.cells[path[n].i][path[n].k].el.append(
        '<div class="map-path-line map-path-line-' + num + '"></div>',
      );
    }
  },
};
...
$(document).on("click", ".map_cell", function (e) {
  ...
  if (selected_unit.select_mission) {
    if ($(e.target).closest(".map_cell").css("cursor") == "crosshair") {
      ...
      path.shift();
      if (selected_unit.select_mission == "build_road_to") {
        selected_unit.build_road_to(path);
      } else {
