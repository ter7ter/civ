...
class EditGameIntegrationTest extends TestBase
{
    ...
    public function testFullGameEditProcess(): void
    {
        ...
        $this->assertFalse(
            $error,
            "Редактирование игры должно пройти без ошибок: " .
                (is_string($error) ? $error : ""),
        );

        // Проверяем, что игра обновлена в "базе данных"
        $this->assertTrue(
            $this->recordExists("game", ["name" => "Отредактированная игра"]),
        );
        $this->assertFalse(
            $this->recordExists("game", ["name" => "Исходная игра"]),
        );
    ...
    public function testEditedGameDataPersistence(): void
    {
        ...
        $this->assertEquals(
            2,
            $userCount,
            "Количество пользователей должно остаться прежним",
        );
    }
    ...
    public function testEditDifferentTurnTypesIntegration(): void
    {
        ...
        foreach ($turnTypes as $turnType => $gameName) {
            ...
            $this->assertFalse(
                $error,
                "Редактирование игры с типом {$turnType} должно пройти успешно: " .
                    (is_string($error) ? $error : ""),
            );
    ...
    public function testPlayerOrderPreservationOnEdit(): void
    {
        ...
        $userOrder = MyDB::query(
            "SELECT login, turn_order FROM user WHERE game = ? ORDER BY turn_order",
            [$game["id"]],
        );
        ...
        for ($i = 0; $i < count($players); $i++) {
            $this->assertEquals(
                $players[$i],
                $userOrder[$i]["login"],
                "Порядок игроков должен сохраниться",
            );
            $this->assertEquals(
                $i + 1,
                $userOrder[$i]["turn_order"],
                "Порядок ходов должен сохраниться",
            );
        }
    }
    ...
    public function testPlayerParametersPreservationOnEdit(): void
    {
        ...
        $users = MyDB::query(
            "SELECT * FROM user WHERE game = ? ORDER BY turn_order",
            [$game["id"]],
        );
    ...
    public function testValidationErrorsIntegrationOnEdit(): void
    {
        ...
        $this->assertEquals(
            "Исходная игра",
            $gameRecord["name"],
            "Название игры не должно измениться при ошибках валидации",
        );
        $this->assertEquals(
            100,
            $gameRecord["map_w"],
            "Ширина карты не должна измениться при ошибках валидации",
        );
        $this->assertEquals(
            100,
            $gameRecord["map_h"],
            "Высота карты не должна измениться при ошибках валидации",
        );
        $this->assertEquals(
            "byturn",
            $gameRecord["turn_type"],
            "Тип ходов не должен измениться при ошибках валидации",
        );
    }
    ...
    public function testEditGameWithRelatedDataPreservation(): void
    {
        ...
        $this->assertEquals(
            5,
            $updatedGame["turn_num"],
            "Номер хода должен сохраниться",
        );
