...
class User {
	...
	public function __construct($data) {
		foreach (['login', 'money', 'income', 'color', 'age', 'game', 'turn_status', 'turn_order',
		...
		if (isset($data['process_research_type']) && $data['process_research_type']) {
			$this->process_research_type = ResearchType::get($data['process_research_type']);
		}
	}
        ...
        foreach ($culture_cells as $x => $cells) {
            foreach ($cells as $y => $culture) {
                ...
                if (!$cell->owner) {
                    ...
                    $cell->owner_culture = (int)$culture;
                    $cell->save();
                } elseif ($cell->owner->id <> $this->id && $cell->owner_culture + 10 < $culture) {
                    if (!$cell->city) {
                        $cell->owner = $this;
                        $cell->owner_culture = (int)$culture;
                        $cell->save();
	...
	public function calculate_research() {
		...
		if ($this->process_research_complete >= $this->process_research_type->cost) {
			//Исследование завершено
			$research = new Research([	'type' => $this->process_research_type->id,
							'user_id' => $this->id]);
			$research->save();
			$event = new Event(['type' => 'research',
                                'user_id' => $this->id,
	...
	public function get_available_research() {
		...
		foreach (ResearchType::$all as $res) {
			if (isset($research[$res->id])) continue;
			if ($res->age > $this->age) continue;
			$ok = true;
			foreach ($res->requirements as $req) {
				if (!isset($research[$req->id])) $ok = false;
			}
			if ($ok) $result[$res->id] = $res;
		}
		return $result;
	}
	...
	public function get_research_need_turns($research = false) {
		...
		$turns = Ceil(($cost - $this->process_research_complete) / $this->research_amount);
    ...
    public function calculate_resource()
    {
        ...
        while (count($cities) > 0) {
            ...
            if (in_array($city->id, $cities_in_group)) continue;
            $group_id++;
            $groups[$group_id] = [
                'cities' => [$city->id => $city],
                'resources' => []
            ...
            while (count($next_cells) > 0) {
                ...
                if (in_array($cell->x.'_'.$cell->y, $cells_in_group)) continue;
                $cells_in_group[] = $cell->x.'_'.$cell->y;
                if ($cell->resource && $cell->resource->type && $cell->resource->type->type <> 'bonuce') {
                    $groups[$group_id]['resources'][] = $cell->resource;
                }
                $cells_around = Cell::get_cells_around($cell->x, $cell->y, 3, 3);
                foreach ($cells_around as $row) foreach($row as $acell) {
                    if ($acell == $cell) continue;
                    if ($acell->owner && $acell->owner <> $this) continue; //Через чужую территорию ничего не идёт
                    if ($acell->city && $acell->owner == $this) { //Наш город, добавим его к группе
                        $groups[$group_id]['cities'][$acell->city->id] = $acell->city;
                        $cities_in_group[] = $acell->city->id;
                        $next_cells[] = $acell;
                        continue;
                    }
                    if ($acell->road) { //Дорога по своей или нейтральной территории
                        $next_cells[] = $acell;
                        continue;
                    }
                    if ($cell->type->id == 'water1' && $acell->type->id == 'water1') { //Водный путь, прибрежный
                        $next_cells[] = $acell;
                        continue;
                    }
                    if ($cell->type->id == 'water2' && ($acell->type->id == 'water2' || $acell->type->id == 'water1')) { //Водный путь, морской
                        $next_cells[] = $acell;
                        continue;
                    }
                    if ($cell->type->id == 'water3' && ($acell->type->id == 'water3' || $acell->type->id == 'water2')) { //Водный путь, по океану
                        $next_cells[] = $acell;
                        continue;
                    }
                    if ($acell->type->id == 'water1' && $cell->city && isset($cell->city->buildings[8])) { //Переход с города с гаванью на воду
                        $next_cells[] = $acell;
                        continue;
                    }
        ...
        foreach ($groups as $group_id => $group) {
            ...
            foreach ($group['cities'] as $city) {
                ...
                if (count($group['resources']) > 0) {
                    MyDB::update('city', ['resource_group' => $group_id], $city->id);
                    foreach ($group['resources'] as $resource) {
                        if (isset($city->resources[$resource->type->id])) {
                            $city->resources[$resource->type->id]['count']++;
                        } else {
                            $city->resources[$resource->type->id] = ['type' => $resource->type, 'count' => 1];
                        }
                    }
                } else {
                    MyDB::update('city', ['resource_group' => 'NULL'], $city->id);
                }
                $city->save();
            }
        }
