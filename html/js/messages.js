var messages = {
  ...
  show_tab: function () {
    ...
    if ($(".message-window-tab-active").attr("id") == "mw-all-messages") {
      ...
      $(".message-window-tab-active").attr("id") == "mw-system-messages"
    ) {
      type = "system";
    } else if (
      $(".message-window-tab-active").attr("id") == "mw-chat-messages"
    ) {
      type = "chat";
    }
    ...
    for (var i in this.all) {
      if (type == "all" || this.all[i].type == type) {
        ...
        $("#message-window-lines").append(
          "<b>" + from + "</b>:" + this.all[i].text + "<br>",
        );
      }
    }
...
map.status_timer = setInterval(function () {
  $.post(
    "index.php?method=message&json=1",
    { last: messages.last },
    function (data) {
      resp = $.parseJSON(data);
      if (resp.status == "ok") {
    ...
    var error_msg =
      "Ошибка при загрузке сообщений: " + textStatus + "\n" + errorThrown;
    if (jqXHR.responseText) {
      try {
        var resp = $.parseJSON(jqXHR.responseText);
        error_msg += "\n\nСерверная ошибка: " + resp.error;
