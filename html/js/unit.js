...
var Unit = {
  ...
  select: function () {
    ...
    $(".map_unit_selected")
      .removeClass("map_unit_selected")
      .addClass("map_unit");
    el.find(".map_unit").remove();
    el.find(".map_unit_selected").remove();
    el.append(
      '<img class="map_unit_selected" src="./img/units/' +
        this.image_file +
        '" style="border-color: ' +
        this.owner_color +
        '"></img>',
  ...
  calculate_path: function (target_x, target_y) {
    ...
    for (var i in map.cells) {
      for (var k in map.cells[i]) {
        if (map.cells[i][k].x == this.x && map.cells[i][k].y == this.y) {
          //Клетка отправки
          i1 = i;
          k1 = k;
        }
        if (map.cells[i][k].x == target_x && map.cells[i][k].y == target_y) {
          //Клетка прибытия
          i2 = i;
          k2 = k;
        }
    ...
    while (cells_next.length > 0) {
      ...
      if (
        cells_path[next.i + "_" + next.k] &&
        cells_path[next.i + "_" + next.k].dist <= next.dist
      ) {
      ...
      for (var di = -1; di < 2; di++)
        for (var dk = -1; dk < 2; dk++) {
          ...
          if (
            next.i * 1 + di < 0 ||
            next.i * 1 + di >= map.width ||
            next.k * 1 + dk < 0 ||
            next.k * 1 + dk >= map.height
          )
            continue;
          var dist_near;
    ...
    if (dist <= map.max_path) {
      ...
      while (!(i == i1 && k == k1)) {
        if (cells_path[i + "_" + k].prev) {
          path.push({
            i: cells_path[i + "_" + k].prev.i,
            k: cells_path[i + "_" + k].prev.k,
          });
