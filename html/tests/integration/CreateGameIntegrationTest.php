...
class CreateGameIntegrationTest extends TestBase
{
    ...
    public function testFullGameCreationProcess(): void
    {
        ...
        $this->assertFalse(
            $error,
            "Создание игры должно пройти без ошибок: " .
                (is_string($error) ? $error : ""),
        );

        // Проверяем, что игра создана в "базе данных"
        $this->assertTrue(
            $this->recordExists("game", ["name" => "Интеграционный тест игры"]),
        );
    ...
    public function testGameDataPersistence(): void
    {
        ...
        $this->assertEquals(
            2,
            $userCount,
            "Должно быть создано 2 пользователя",
        );
    }
    ...
    public function testDifferentTurnTypesIntegration(): void
    {
        ...
        foreach ($turnTypes as $turnType => $gameName) {
            ...
            $this->assertFalse(
                $error,
                "Создание игры с типом {$turnType} должно пройти успешно: " .
                    (is_string($error) ? $error : ""),
            );
            ...
            $this->assertEquals(
                $turnType,
                $gameRecord["turn_type"],
                "Тип ходов должен быть {$turnType}",
            );
        }
    }
    ...
    public function testUniquePlayerColors(): void
    {
        ...
        $this->assertFalse(
            $error,
            "Создание игры должно пройти успешно: " .
                (is_string($error) ? $error : ""),
        );

        // Проверяем, что все цвета уникальны
        $colors = MyDB::query("SELECT color FROM user");
        $colors = array_column($colors, "color");

        $this->assertEquals(
            count($players),
            count($colors),
            "Должно быть создано столько цветов, сколько игроков",
        );
        $this->assertEquals(
            count($colors),
    ...
    public function testPlayerTurnOrder(): void
    {
        ...
        $userOrder = MyDB::query(
            "SELECT login, turn_order FROM user ORDER BY turn_order",
        );
        ...
        for ($i = 0; $i < count($players); $i++) {
            $this->assertEquals(
                $players[$i],
                $userOrder[$i]["login"],
                "Порядок игроков должен соответствовать порядку в массиве",
            );
            $this->assertEquals(
                $i + 1,
                $userOrder[$i]["turn_order"],
                "Порядок ходов должен начинаться с 1",
            );
        }
    }
    ...
    public function testInitialPlayerParameters(): void
    {
        ...
        foreach ($users as $user) {
            ...
            $this->assertEquals(
                50,
                $user["money"],
                "Начальные деньги должны быть 50",
            );
            $this->assertEquals(1, $user["age"], "Начальная эра должна быть 1");
            $this->assertEquals(
                0,
                $user["income"],
                "Начальный доход должен быть 0",
            );
        }
    }
    ...
    public function testValidationErrorsIntegration(): void
    {
        ...
        $this->assertEquals(
            ...
            "Игра не должна быть создана при ошибках валидации",
        );
        $this->assertEquals(
            0,
            $this->getTableCount("user"),
            "Пользователи не должны быть созданы при ошибках валидации",
        );
    }
    ...
    public function testPerformanceWithMaxPlayers(): void
    {
        ...
        $this->assertEquals(
            1,
            $this->getTableCount("game"),
            "Должна быть создана 1 игра",
        );
        $this->assertEquals(
            16,
            $this->getTableCount("user"),
            "Должно быть создано 16 игроков",
        );
    }
    ...
    public function testSQLInjectionProtection(): void
    {
        ...
        $this->assertContains(
            "game",
            $tables,
            "Таблица game должна существовать",
        );
        $this->assertContains(
            "user",
            $tables,
            "Таблица user должна существовать",
        );
        ...
        if (!$error) {
            ...
            if ($gameRecord) {
                $this->assertEquals(
                    htmlspecialchars($maliciousData["name"]),
                    $gameRecord["name"],
                );
            }
        }
    ...
    public function testMultipleGameCreation(): void
    {
        ...
        $this->assertEquals(
            3,
            $this->getTableCount("game"),
            "Должно быть создано 3 игры",
        );
        $this->assertEquals(
            9,
            $this->getTableCount("user"),
            "Должно быть создано 9 игроков всего",
        );
    }

    // Моки теперь реализованы в отдельных файлах и подключаются через MockLoader.php
}
